...
# 根据拼音字典构建高频字集合
def build_common_chars():
    """
    从拼音字典中提取高频使用的汉字
    字典中的汉字是按使用频率由低到高排序的，所以排除低频字
    """
    # 提取所有汉字
    all_chars = set()
    for chars in pinyin_dict_notone.values():
        all_chars.update(list(chars))
    
    # 构建字符频率评估函数
    def char_complexity(char):
        """评估字符复杂度，返回复杂度分数，越高表示越复杂"""
        # 使用Unicode范围大致判断复杂度
        unicode_val = ord(char)
        
        # 常用字区域
        if 0x4e00 <= unicode_val <= 0x9fff:  # CJK统一汉字基本区
            return 1
        elif 0x3400 <= unicode_val <= 0x4dbf:  # CJK扩展A区
            return 2
        elif 0x20000 <= unicode_val <= 0x2a6df:  # CJK扩展B区
            return 3
        elif 0xf900 <= unicode_val <= 0xfaff:  # CJK兼容汉字区
            return 2
        else:
            return 1
    
    # 根据字典顺序，前面的字是低频字，后面的是高频字
    # 我们保留高频字，过滤低频字
    high_freq_chars = set()
    low_freq_chars = set()
    
    # 按照拼音分组统计，根据位置判断频率
    for pinyin, chars in pinyin_dict_notone.items():
        char_list = list(chars)
        # 根据用户反馈，前面的字是高频字，后面的字是低频字
        # 将前半部分视为高频字
        split_point = max(1, len(char_list) // 2)  # 取前一半作为高频字
        high_freq_chars.update(char_list[:split_point])
        low_freq_chars.update(char_list[split_point:])
    
    # 返回低频和复杂字符集合
    uncommon_chars = set()
    
    # 添加低频字
    uncommon_chars.update(low_freq_chars)
    
    # 添加高复杂度字
    for char in all_chars:
        if char_complexity(char) >= 3:  # 高复杂度字符
            uncommon_chars.add(char)
    
    return uncommon_chars, high_freq_chars

# 动态构建不常见字符集合
UNCOMMON_CHARS, COMMON_CHARS = build_common_chars()
...
def calculate_word_frequency_score(word):
    """
    计算词语的频率得分，得分越低表示越不常用
    """
    score = 0
    for char in word:
        if char in COMMON_CHARS:
            score += 1  # 高频字加分
        elif char in UNCOMMON_CHARS:
            score -= 1  # 低频字减分
        else:
            score -= 0.5  # 不在字典中的字减分
    return score
...
def filter_words(file_path):
    """
    过滤词条，仅保留二字词和三字词中频率得分最高的前 50%。
    其他词条直接保留（保持原有的过滤逻辑可在后续扩展）。
    """
    # 读取全部行，去除结尾换行符
    ...
    for line in lines:
        ...
        word = parts[0]
        score = calculate_word_frequency_score(word)

        if len(word) == 2:
            two_char.append((score, line))
        elif len(word) == 3:
            three_char.append((score, line))
        else:
            others.append(line)

    # 按得分降序排列
    two_char.sort(key=lambda x: x[0], reverse=True)
    three_char.sort(key=lambda x: x[0], reverse=True)

    def top_half(collection):
        """返回得分最高的前 50%（向上取整）行的原始文本列表。"""
        n = len(collection)
        keep = (n + 1) // 2   # 向上取整的 50%
        return [line for _, line in collection[:keep]]

    # 合并保留的行
    kept_lines = others + top_half(two_char) + top_half(three_char)
...
def main():
    ...
    print(f"总共包含 {len(UNCOMMON_CHARS)} 个不常见字符，{len(COMMON_CHARS)} 个常见字符")
