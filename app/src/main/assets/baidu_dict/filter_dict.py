...
def filter_words(file_path):
    ...
    for line in lines:
        ...
        # 计算频率得分
        freq_score = calculate_word_frequency_score(chinese_word)
        
        # 根据长度、不常见字符和频率得分进行过滤
        # 如果词语长度小于5，不包含不常见字符，且频率得分相对较高，则保留
        should_keep = word_length < 5 and not has_uncommon_char and freq_score >= 0
        
        # 对于较长的词，如果全部由高频字组成，也可以考虑保留
        if word_length >= 5 and word_length <= 8 and freq_score > word_length / 2:
            should_keep = True
